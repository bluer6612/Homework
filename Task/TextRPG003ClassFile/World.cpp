...
void UWorld::PlayerZonePlay(class UPlayer& _Player)
{
	...
	TownZone0.SetNumber(0);

	UTown TownZone1;
	TownZone1.SetName("중급마을");
	TownZone1.SetNumber(1);

	UFightZone FightZone;
	FightZone.SetName("초보사냥터");
	FightZone.SetNumber(2);

	int Select = TownZone0.InPlayer(_Player, TownZone0.GetNumber());
	...
	while (true)
	{
		...
		{
		case 0:
			Select = TownZone0.InPlayer(_Player, Select);
			break;
		case 1:
			Select = TownZone1.InPlayer(_Player, Select);
			break;
		case 2:
			Select = FightZone.InPlayer(_Player, Select);
			break;
		default:
			break;
		}
